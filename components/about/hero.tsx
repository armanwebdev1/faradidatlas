  ...
  return (
    <>
      ...
      <section className="relative h-screen w-full overflow-hidden bg-neutral-950">
        ...
        <div className="relative h-full w-full flex flex-col justify-end" ref={containerRef}>
          <div className={`max-w-5xl px-8 sm:px-12 md:px-16 lg:pl-32 lg:pr-8 pb-20 md:pb-28 ${isRTL ? "text-right" : "text-left"}`}>
            <div className="animate-fade-in-up">
              ...
              <p 
                ...
                style={{
                  fontFamily: lang === "en" ? "var(--font-body)" : "Shabnam, var(--font-body)",
                  textAlign: isRTL ? "right" : "left",
                }}
              >
                {lang === "en"
      ...
      <section className="relative py-20 md:py-32 px-4 sm:px-6 bg-white overflow-hidden">
        ...
        <div className="max-w-7xl mx-auto">
          <div className={`grid grid-cols-1 lg:grid-cols-2 gap-12 lg:gap-16 items-center ${isRTL ? "lg:flex-row-reverse" : ""}`}>
            ...
            {/* Text Content */}
            <div className={`space-y-8 animate-fade-in-up ${isRTL ? "lg:order-1" : ""}`}>
              <div>
                <h2 className="text-4xl md:text-5xl font-bold text-primary mb-6 tracking-tight font-hero" style={{ textAlign: isRTL ? "right" : "left" }}>
                  {lang === "en" ? "Our Legacy" : "میراث ما"}
                </h2>
                <div className="divider-premium w-16 h-1 mb-8" style={{ marginLeft: isRTL ? "auto" : "0", marginRight: isRTL ? "0" : "auto" }} />
              </div>

              <div className="space-y-6">
                <p className="text-lg text-gray-700 leading-relaxed font-light" style={{ textAlign: isRTL ? "right" : "left" }}>
                  {lang === "en"
            ...
            {[
              {
                ...
                description: lang === "en" ? "Trusted partnerships since 1998" : "شرکای قابل اعتماد از سال ۱۳۷۷",
              },
              {
                number: "40+",
                label: lang === "en" ? "Countries Served" : "کشور سرویس‌شده",
                description: lang === "en" ? "Global reach with local expertise" : "دسترسی جهانی با تخصص محلی",
              },
              {
                number: "100%",
                label: lang === "en" ? "Quality Commitment" : "تعهد کیفیت",
                description: lang === "en" ? "Zero compromise on standards" : "بدون سازش در استانداردها",
              },
            ].map((stat, idx) => (
              <div key={idx} className="group text-center animate-fade-in-up p-8 rounded-xl hover:bg-white hover:shadow-lg transition-all duration-500 cursor-pointer">
                ...
                <h3 className="text-xl md:text-2xl font-bold text-primary mb-2">{stat.label}</h3>
                <p className="text-gray-600 text-sm md:text-base leading-relaxed">{stat.description}</p>
              </div>
            ))}
          </div>
        </div>
