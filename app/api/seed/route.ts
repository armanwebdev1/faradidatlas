...
export async function POST(request: Request) {
  try {
        ...
        valueProps: [
          {
            ...
            description: { en: 'First-tier supplier relationships in key food-producing markets, including India and Pakistan.', fa: 'با تأمین‌کنندگان معتبر در بازارهای اصلی تولید غذا، از جمله چین، هند، و پاکستان، همکاری می‌کنیم.' },
            isActive: true,
          },
          {
            icon: 'CheckCircle',
            title: { en: 'Quality Before Volume', fa: 'کیفیت؛ شرط اول همکاری' },
            description: { en: 'Products are selected against international hygiene and quality standards such as ISO 22000.', fa: 'پیش از هر همکاری، محصول از نظر سلامت، کیفیت و ثبات بررسی می‌شود.' },
            isActive: true,
          },
          {
            icon: 'Route',
            title: { en: 'Stable Supply Channels', fa: 'تأمین منظم برای خرید عمده' },
            description: { en: 'Regional branches, offices, and warehouses support continuous supply for B2B buyers.', fa: 'با شبکه منطقه‌ای دفتر، شعبه و انبار، سفارش‌های عمده و سازمانی را پشتیبانی می‌کنیم.' },
            isActive: true,
          },
          {
            icon: 'Package',
            title: { en: 'Accessible Buyer Channels', fa: 'مسیر ساده‌تر برای خرید' },
            description: { en: 'Offices, regional operations, and digital sales channels support individuals, wholesalers, organizations, and foodservice buyers.', fa: 'از تماس حضوری تا فروش دیجیتال، مسیر خرید را ساده و سریع نگه می‌داریم.' },
            isActive: true,
          },
        ],
        brandsSection: {
    ...
    return NextResponse.json({ success: true, message: 'Homepage seeded successfully', doc: { id: result.doc.id } })
  } catch (error) {
    console.error('Seed error:', error)
    return NextResponse.json({ error: 'Failed to seed homepage', details: String(error) }, { status: 500 })
  }
}
